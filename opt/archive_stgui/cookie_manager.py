...
from authentication.auth_exceptions import AuthCookieError, AuthExceptionLogger
...
class CookieManager:
    ...
    def _token_encode(self) -> str:
        ...
        try:
            ...
            return jwt.encode(
                {"uid": uid, "username": username, "exp_date": self.exp_date},
                self.key,
                algorithm="HS256",
            )
                ...
                if token is not False:
                    if token["exp_date"] > datetime.utcnow().timestamp():
                        if "uid" and "username" in token:
                            self.logger.debug(
                                "Cookie found, logging in with username %s",
                                token["username"],
                            )
